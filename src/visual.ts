...
export class Visual implements IVisual {
  ...
  public update(options: VisualUpdateOptions) {
    if (!options || !options.dataViews || !options.dataViews[0] || _.isNil(this.graph)) {
      return
    }
    ...
    if (this.data.categorical && this.data.categorical.categories) {
      ...
      this.graph.nodes.forEach((n: INode) => {
        n.layout.x = Math.random() * 300 // tslint:disable-line:insecure-random
        n.layout.y = Math.random() * 300 // tslint:disable-line:insecure-random
      })
